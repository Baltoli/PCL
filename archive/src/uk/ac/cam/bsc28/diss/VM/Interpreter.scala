...
import uk.ac.cam.bsc28.diss.CodeGeneration.LabelGenerator
...
class Interpreter(program: List[Instruction],
                  ...
                  parent: Option[Interpreter], newID: Long) extends Runnable {
  ...
  def execute(i: Instruction): Unit = {
    Config.traceLine(s"Thread $ID: $i")
    i match {
      ...
      case Fresh(vn) =>
        val name = LabelGenerator.nextLabel()
        environment += (vn -> Left(Channel("§" + name)))
